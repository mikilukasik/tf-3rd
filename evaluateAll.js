...
const reTrainingPattern = '451_r';

const evalDirName = path.resolve(`evaluuations/${_modelDirName.split('/').pop()}`);
...
const loadRetrainingMetas = async () => {
  const modelRootFolder = path.resolve(_modelDirName.split('/').slice(0, -1).join('/'));
  const retrainingFolders = (await fs.readdir(modelRootFolder)).filter((folder) =>
    folder.startsWith(reTrainingPattern),
  );

  for (const folder of retrainingFolders) {
    retrainingMetas[folder] = JSON.parse(await fs.readFile(path.resolve(modelRootFolder, folder, 'trainingMeta.json')));
  }
};
...
const getCachedMAE = async ({ modelIndex, iterationIndex, retrainingKey }) => {
  try {
    return JSON.parse(
      await fs.readFile(
        path.resolve(evalDirName, `${retrainingKey ? `r${retrainingKey}_` : ''}m${modelIndex}i${iterationIndex}.json`),
        'utf8',
      ),
    );
  } catch (e) {
...
const cacheMAE = async ({ modelIndex, iterationIndex, meanAbsoluteError, retrainingKey }) => {
  await fs.writeFile(
    path.resolve(evalDirName, `${retrainingKey ? `r${retrainingKey}_` : ''}m${modelIndex}i${iterationIndex}.json`),
    JSON.stringify({ modelIndex, iterationIndex, meanAbsoluteError, _modelDirName }),
    'utf8',
  );
};

const evaluateIteration = async ({
  iterationIndex,
  modelIndex,
  modelEvaluations,
  originalMeanAbsError,
  retrainingKey,
}) => {
  const cachedData = await getCachedMAE({ modelIndex, iterationIndex, retrainingKey });
  if (cachedData) {
    const { meanAbsoluteError } = cachedData;
    modelEvaluations[iterationIndex] = meanAbsoluteError;
    return;
  }
  ...
  await cacheMAE({ modelIndex, iterationIndex, meanAbsoluteError, retrainingKey });
};

// run
const run = async function () {
  await fs.mkdir(evalDirName, { recursive: true });

  await loadTrainingMeta();
  await loadRetrainingMetas();

  // console.log(retrainingMetas);
  // process.exit(0);

  const iterations = trainingMeta.iterations.slice();

  for (const retrainingMetaKey of Object.keys(retrainingMetas)) {
    iterations.push(
      ...retrainingMetas[retrainingMetaKey].iterations.map((iteration) =>
        Object.assign({}, iteration, { retrainingKey: retrainingMetaKey }),
      ),
    );
  }

  let modelIndex = 0;
  while (modelIndex < iterations.length) {
    const originalMeanAbsError = iterations[modelIndex].meanAbsoluteError;

    await loadModel({ folder: iterations[modelIndex].folder });
    model.compile({
      optimizer: 'adam',
    ...
    while (iterationIndex < trainingMeta.iterations.length) {
      await evaluateIteration({
        ...
        modelEvaluations,
        originalMeanAbsError,
        retrainingKey: iterations[modelIndex].retrainingKey,
      });
      iterationIndex += 1;
    }

    console.log(
      `${iterations[modelIndex].folder}: ${getAverage(modelEvaluations.map(Number)).toFixed(
        5,
      )} using ${modelEvaluations.join(', ')}`,
    );
...
//     await loadModels()//({ folder: latestModelFolder });
//     // model.compile({
//     //   optimizer: 'adam',
//     //   loss: 'meanSquaredError',
//     //   metrics: [tf.metrics.meanAbsoluteError],
