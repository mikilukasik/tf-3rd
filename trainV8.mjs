...
const targetModelName = 'models/small_tV8_mUpUbU_v1';
...
const getIsDupe = () => {
  ...
  return (record) => {
    ...
    if (dupeCache[fen][1] > value) return true; // existing is better

    if (dupeCache[fen][0] === move) {
      // same move, but new val is better
      dupeCache[fen][1] = value; //update existing value
      return true;
    }

    // same fen, different move with a better value
    dupeCache[fen] = [move, value];
    return false;
  };
};
...
const loadTestData = async () => {
  ...
  const rawTestData = await getNextDatasets({ batchSize: testRecordsPerDataset, isDupe: getIsDupe() });
