...
const buildModel = function () {
  ...
  const conv3b = convBlock({ input: conv3a, kernelSize: 3, filters: 256 });

  const conv8a = convBlock({ input, kernelSize: 8, filters: 32 });
  const conv8b = convBlock({ input: conv8a, kernelSize: 8, filters: 256 });

  const flatInput = flattenLayer({ input });
  const flat3 = flattenLayer({ input: conv3b });
  const flat8 = flattenLayer({ input: conv8b });

  const concatenated = concatLayer([flatInput, flat3, flat8]);

  const dense1 = denseLayer({ input: concatenated, units: 10000 });
  const dense2 = denseLayer({ input: dense1, units: 5000 });
