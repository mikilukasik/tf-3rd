...
const targetModelName = 'models/small_tV7_mUpUbU_v1';
...
const loadTestData = async () => {
  ...
  const rawTestData = await getNextDatasets({ batchSize: testRecordsPerDataset });
...
const runIteration = async ({ model, iterationIndex }) => {
  ...
  const rawTrainData = await getNextDatasets();
  if (!rawTrainData) {
    ...
    if (!alreadyInited)
      getNextDatasets = await (async () => {
        ...
        return async (options) => {
          ...
          console.log(`Loaded ${records.length} records. ${options ? `\n${JSON.stringify(options, null, 2)}` : ''}`);
          return records;
        };
      })();
