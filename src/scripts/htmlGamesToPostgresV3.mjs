...
import pkg from 'pg';
import { addWNextFenV3 } from './utils/addWNextFenV3.mjs';
import { addFlippedAndRotatedV4 } from './utils/addFlippedAndRotatedV4.mjs';
const { Client } = pkg;
...
const getRecords = async ({ fens, filename }) => {
  ...
  // const recordsWithMoveIndicators = ;
  ...
  return {
    records: addFlippedAndRotatedV4(records.map(addWNextFenV3)), //: newRecords.map(pruneRecord),
    // chkmate_ending,
    // stall_ending,
    // aborted_ending,
    // total_moves,
...
const summarizeRecords = (recordsInput) => {
  ...
  recordsInput.forEach((record) => {
    ...
    ['progress_total', 'count'].forEach((key) => {
...
const getUpdateQuery = (inputArr, keys, matcher) =>
  `UPDATE mytable as t set ${keys.map((key) => `${key}=t.${key} + c.${key}`).join(',')} from (values ${inputArr
    .map((element) => `('${element[matcher]}', ${keys.map((k) => recordValues[k](element)).join(',')})`)
    .join(',')} ) as c(${matcher},${keys.join(',')}) where c.${matcher}=t.${matcher};`;
...
const run = async () => {
  ...
  for (let nextGames = await getNextGames(); nextGames && nextGames.length; nextGames = await getNextGames()) {
    ...
    const allRecords = nextGames.map(({ game: { records } }) => records).flat();
    const allFilenames = nextGames.map(({ game: { filename } }) => filename);
    const records = summarizeRecords(allRecords); //.slice(-1);
    // const moves = summarizeMoves(allRecords); //.slice(-1);

    const updateQuery = getUpdateQuery(records, recordKeys, 'fen');
    console.log({ updateQuery });
    const pgRecordsPromise = pgClient.query(updateQuery);
