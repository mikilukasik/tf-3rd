...
const layerNamePrefix = 'tw_ch';
const targetModelDirName = `models/tweakedChampion`;
...
const createDenseModel = () => {
  ...
  const flatLayer = flattenLayer({ input });

  const dense1 = denseLayer({ input: flatLayer, units: 1024 });
...
const sourceModels = {
  // // 'models/newest_M_tV13-p12_v1_0.0000125/2.15299225-1676476410289'
  // 'models/newest_SM_tV13-p0_v1_0.0000015625/1.65538979-1676396035293': {
  //   //'models/newest_SM_tV13-p0_v1_0.0001/1.70007908-1676228694116': {
  //   createModel: createSMModel,
  // },
  // // 'models/newest_M_tV13-p12_v1_0.0000125/2.15299225-1676476410289': {
  // 'models/newest_M_tV13-p12_v1_0.0000125/2.15299225-1676476410289': {
  //   createModel: createMModel,
  // },
  // // 'models/newest_SM_tV13-p3_v1_0.00005/2.02389336-1676230696102': {
  // 'models/newest_SM_tV13-p3_v1_0.0000125/1.99288964-1676354578425': {
  //   createModel: createSMModel,
  // },
  // 'models/newest_tV13-progress_v1_0.000005/0.07604351-1676229068377': {
  //   createModel: createPModel,
  // },
  // 'models/newest_tV13-12_v1_0.000001953125/0.07416017-1676463799075': {}, //tprogress
  'models/champion_1.95': {
    cutLayersFromEnd: 2,
  },
};

const attachLayersToNewInputLayer = ({
  layers,
  input,
  ...
  const copiedLayers = layers.slice(1, -1 * cutLayersFromEnd);
...
const copyModel = async ({ sourceModelDirName, sourceModel }) => {
  if (!sourceModels[sourceModelDirName].createModel) return sourceModel;

  const blankModel = sourceModels[sourceModelDirName].createModel();
  blankModel.summary();
...
const getConcatenatedLayers = async ({ input }) => {
  ...
  for (const sourceModelDirName of Object.keys(sourceModels)) {
    ...
    const attachedLayers = attachLayersToNewInputLayer({
      layers: copiedModel.layers,
      input,
      ...sourceModels[sourceModelDirName],
    });
    lastUsedLayers.push(attachedLayers[attachedLayers.length - 1]);
  }

  // const denseModel = createDenseModel();
  // const newDLayers = attachLayersToNewInputLayer({
  //   layers: denseModel.layers,
  //   input,
  //   trainable: true,
  // });
  // lastUsedLayers.push(newDLayers[newDLayers.length - 1]);

  return lastUsedLayers.length > 1 ? concatLayer(lastUsedLayers) : lastUsedLayers[0];
};

const getTweakedModel = async () => {
  const input = tf.layers.input({ shape: [8, 8, inUnits] });

  const concatenatedLastLayer = concatLayer([await getConcatenatedLayers({ input }), flattenLayer({ input })]);

  const newDenseUnits = Math.ceil((concatenatedLastLayer.shape[1] * 2 + outUnits) / 3);

  const dense1 = denseLayer({ input: concatenatedLastLayer, units: newDenseUnits });
  const dense2 = denseLayer({ input: dense1, units: newDenseUnits });
