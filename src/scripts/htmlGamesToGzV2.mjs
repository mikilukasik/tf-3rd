...
const {
  ...
  groups: ({
    ...
    // const smba = balAhead.slice(0, 10).map((n) => Math.floor(n / 100));

    // console.log(smba);

    // console.log(balAhead);

    // console.log({ nextBalance, nextBalanceDistance });
    ...
    const transform = () => {
      ...
      return [
        ...
        chkmate_ending || stall_ending ? progress : progress * 0.8, // adjusted progress for games that were not completed
        w_rating,
        b_rating,
        min_rating_diff,
        max_rating_diff,
