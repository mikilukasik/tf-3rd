...
const sourceModelDirName = 'models/inc_rs_v2_0.000015625/2.18603182-1665416717914';
const layerNamePrefix = 'v2.5';

const targetModelDirName = `models/inc_fixed_${layerNamePrefix}`;
...
//   const conv3b = convBlock({ input: conv3a, kernelSize: 3, filters: 26, layerNamePrefix });
...
//       useBias: false,
//       name: `${layerNamePrefix}__dense_leakyReLU_u${896}-${Math.random().toString().slice(2)}`,
//     })
//     .apply(flat3b);
...
const getTweakedModel = ({ sourceModel }) => {
  ...
  const copiedLayers = sourceModel.layers.slice(0, -5);
  ...
  console.log([copiedLayers[copiedLayers.length - 1], copiedLayers[copiedLayers.length - 2]]);

  const concatenatedLastLayer = tf.layers
    .concatenate({
      name: `${layerNamePrefix}__concatenate-${Math.random().toString().slice(2)}`,
    })
    .apply([copiedLayers[copiedLayers.length - 1].output, copiedLayers[copiedLayers.length - 2].output]);

  const newDenseLayer1 = tf.layers
    .dense({
      units: Math.ceil(1792 * 1.5 * 1.5),
      activation: tf.leakyReLU,
      useBias: false,
      name: `${layerNamePrefix}__dense_leakyReLU_u${Math.ceil(1792 * 1.5 * 1.5)}-${Math.random().toString().slice(2)}`,
    })
    .apply(concatenatedLastLayer);

  const newDenseLayer2 = tf.layers
    .dense({
      units: Math.ceil(1792 * 1.5),
      activation: tf.leakyReLU,
      useBias: false,
      name: `${layerNamePrefix}__dense_leakyReLU_u${Math.ceil(1792 * 1.5)}-${Math.random().toString().slice(2)}`,
    })
  ...
  const tweakedModel = tf.model({ inputs: copiedLayers[0].input, outputs: outputLayer });
  return tweakedModel;
};
