...
// const mirrorMoveIndexesOnX = (moveIndexes) => {
//   if (!moveIndexes) return moveIndexes;

//   const [from, to, piece] = moveIndexes;

//   return `${mirroredColumns[sc]}${sr}${mirroredColumns[tc]}${tr}${pp}`;
// };

const mirrorLmfLmtOnX = (lmx) => {
  const newLmx = [];
  for (let i = 0; i < lmx.length; i += 8) {
    const chunk = lmx.slice(i, i + 8);
    newLmx.push(...chunk.reverse());
  }
  return newLmx;
};
...
const getRotator = () => {
  ...
  return (record) => {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      resultingRecords.push({ ...rotate90(recordToRotate), version: version++ });
    }
...
export const addFlippedAndRotatedV2 = (records) => {
  const newRecords = [];
  ...
  for (const record of records) {
    ...
    const getRotatedRecords = getRotator();
