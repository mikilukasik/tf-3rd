...
const readFromGroup = async ({
  ...
  if (!pointers[pointerKey]) {
    ...
    console.log(`starting to read dataset from file ${fileName} for ${pointerKey}`);
  ...
  while (remaining && pointers[pointerKey].fileName) {
    ...
    // console.log(`got ${result.length} / ${take} records for ${pointerKey}`);
  }
  // console.log(`----------------  got ${result.length} records for ${pointerKey}`);
...
export const datasetReader = async ({
  ...
  const getNextBatch = async () => {
    ...
    const result = (
      await Promise.all(
        Object.keys(groups).map((pointerKey) =>
          readFromGroup({
            pointers,
            pointerKey, //: test ? 'test-true' : 'test-false',
            take: Math.ceil(groups[pointerKey].take * batchSize), //batchSize,
            folder,
            filter: groups[pointerKey].filter,
            // isDupe,
            beginningToEnd,
            singleMoveRatio,
            singleProgressGroupRatio,
