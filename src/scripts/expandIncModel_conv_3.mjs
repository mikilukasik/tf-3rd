...
const sourceModelDirName = 'models/inc_rs_v2_0.000015625/2.18458986-1665417528890';
const layerNamePrefix = 'v3s';
...
const getLastNewLayer = ({ input }) => {
  ...
  const conv3b = convBlock({ input: conv3a, kernelSize: 4, filters: 26, layerNamePrefix });
...
const getTweakedModel = ({ sourceModel }) => {
  ...
  const copiedLayers = sourceModel.layers.slice(1, -7);
  ...
  const concatenatedLastLayer = tf.layers
    ...
    .apply([copiedLayers[copiedLayers.length - 1], copiedLayers[copiedLayers.length - 2], lastNewLayer]);
  ...
  const newDenseLayer1 = tf.layers
    .dense({
      ...
      activation: tf.leakyReLU,
      useBias: false,
      name: `${layerNamePrefix}__dense_leakyReLU_u${256}-${Math.random().toString().slice(2)}`,
    })
    .apply(flat3b);
