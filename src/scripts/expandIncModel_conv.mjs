...
const sourceModelDirName = 'models/inc_fixed_1_4.8828125e-7';
const layerNamePrefix = 'v2';
...
const getTweakedModel = ({ sourceModel }) => {
  ...
  const newDenseLayer1 = tf.layers
    .dense({
      ...
      activation: tf.leakyReLU,
      useBias: false,
      name: `${layerNamePrefix}__dense_leakyReLU_u${1792}-${Math.random().toString().slice(2)}`,
    })
