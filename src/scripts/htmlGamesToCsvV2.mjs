    ...
    return [
      {
        ...
        // filter,
        getPath: () =>
          `${getProgressGroupName(progress)}/${Math.floor(Math.random() * 50)
            .toString()
            .padStart(2, '0')}`,
        // () => {
...
const run = async () => {
  ...
  for (let nextGames = await getNextGames(); nextGames && nextGames.length; nextGames = await getNextGames()) {
    ...
    const recordTakes = elapsed / processed;
    const remainingTakes = (validFilesCount - processed) * recordTakes;

    console.log(
      `processed ${processed} of ${validFilesCount} games. ETA: ${(remainingTakes / 3600000).toFixed(2)} hours`,
    );
  }
