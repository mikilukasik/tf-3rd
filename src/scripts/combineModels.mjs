...
const layerNamePrefix = 'pg1_sml_v4';
const targetModelDirName = `models/pg1_sml_v4`; //_${layerNamePrefix}`;

const inUnits = 14;
const outUnits = 1837; // 1792 moves where queen promotion is default. 44 knight promotion moves + 1 resign

// const lastLayerTest = (str) => /flatten/.test(str);
...
const getTweakedModel = async () => {
  ...
  const output = outputLayer({ input: concatenatedLastLayer });
