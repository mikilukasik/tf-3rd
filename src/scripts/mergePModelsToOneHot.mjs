...
const targetModelDirName = 'models/oneHot_fixed_v1';
...
const getTweakedModel = ({ sourceModels, progressModel }) => {
  ...
  const concatenatedLastLayer = tf.layers
    .concatenate({
      name: `concat_${Math.random().toString().slice(2)}`,
    })
    .apply(paralellLayers.map((layers) => layers[layers.length - 1]));

  const newDenseLayer1 = tf.layers
    .dense({
      units: oUnits * 1.5 * 1.5,
      activation: tf.leakyReLU,
      useBias: false,
      name: `additional_d1_${Math.random().toString().slice(2)}`,
    })
  ...
  const newDenseLayer2 = tf.layers
    .dense({
      ...
      activation: tf.leakyReLU,
      useBias: false,
      name: `additional_d2_${Math.random().toString().slice(2)}`,
    })
  ...
  const outputLayer = tf.layers
    .dense({
      ...
      activation: 'softmax',
      useBias: false,
      name: `additional_out_${Math.random().toString().slice(2)}`,
    })
