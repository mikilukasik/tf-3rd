...
export default async ({ tf, modelUrl }) => {
  ...
  const getXs = ({ fens, lmt, lmf, noTensor }) => {
    ...
    if (noTensor) return jsArr;

    return tf.tensor(jsArr, [1, 8, 8, 14]);
  };
  ...
  const gameToXs = ({ game: { board, allPastFens, wNext, moves, lmf, lmt } }) => {
    ...
    if (lmf && lmt) {
      return getXs({ fens, lmf, lmt, noTensor: true });
    }
  ...
  const predict = async ({ game }) => {
    ...
    const prediction = j.prediction.split(',').map(Number);
