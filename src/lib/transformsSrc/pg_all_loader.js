...
const modelConfig = [
  { name: 'small', url: 'http://localhost:3300/models/pg_small/model.json' },
  ...
  { name: 'tiny', url: 'http://localhost:3300/models/pg_tiny/model.json' },
  { name: 'tiny_v2', url: 'http://localhost:3300/models/pg_tiny_v2/model.json' },
];

export default async ({ tf, modelUrl }) => {
  console.log('Loading pg all model...');
  const models = [];
  for (const { name, url } of modelConfig) models[name] = await tf.loadLayersModel(url);

  console.log('initialising pg all model transforms.');
