...
export default async ({ tf, modelUrl }) => {
  ...
  console.log('initialising pg small+large model transforms.');
  ...
  const predict = async ({ game }) => {
    ...
    const multiYs = await Promise.all([
      predictWithModel({ xs, modelName: 'small' }),
      predictWithModel({ xs, modelName: 'large' }),
    ]);
