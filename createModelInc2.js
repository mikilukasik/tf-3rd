...
const creatorFilename = 'createModelInc2.js';

const versionName = 'v1';
const modelName = 'inc2';
...
const buildModel = function () {
  ...
  const conv3a = convBlock({ input, kernelSize: 2, filters: 13 });
  const conv3b = convBlock({ input: conv3a, kernelSize: 2, filters: 26 });

  const flat3b = flattenLayer({ input: conv3b });

  const dense1 = denseLayer({ input: flat3b, units: 128 });
  const dense2 = denseLayer({ input: dense1, units: 64 });
