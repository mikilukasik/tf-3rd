...
const buildModel = function () {
  ...
  const dense1 = denseLayer({ input: concatLayer([flatInput, flat3, flat8]), units: 3000 });

  const denseFrom = denseLayer({ input: dense1, units: 1000 });
  const denseTo = denseLayer({ input: dense1, units: 1000 });

  const softMaxFrom = denseLayer({ input: concatLayer([flatInput, denseFrom]), units: 64, activation: 'softmax' });
  const softMaxTo = denseLayer({ input: concatLayer([flatInput, denseTo]), units: 64, activation: 'softmax' });
