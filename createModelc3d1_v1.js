...
const buildModel = function () {
  ...
  const conv8c = convBlock({ input: conv8b, kernelSize: 8, filters: 512 });

  const flatInput = flattenLayer({ input });
  // const flat3 = flattenLayer({ input: conv3b });
  const flat8 = flattenLayer({ input: conv8c });

  const dense1 = denseLayer({ input: concatLayer([flatInput, /*flat3,*/ flat8]), units: 256 });
