...
const conv = ({ filters, kernelSize }) =>
  tf.layers.conv2d({
    ...
    kernelInitializer: tf.initializers.randomUniform({ minval: 0.1, maxval: 0.9 }),
  });
...
const buildModel = function () {
  ...
  const convConcat = concatLayer([conv3a, conv8a]);

  const convB = convBlock({ input: convConcat, kernelSize: 8, filters: 96 });

  // const flat3 = flattenLayer({ input: conv3b });
  // const flat8 = flattenLayer({ input: conv8b });

  const flat = flattenLayer({ input: convB });

  // const concatenated = concatLayer([flat3, flat8]);

  const dense0 = denseLayer({ input: flat, units: 1024 });
  const dense1 = denseLayer({ input: dense0, units: 512 });
  const dense2 = denseLayer({ input: dense1, units: 256 });
