...
const datasetDirName = 'data/datasets/1st_42';

const sourceModelDirName = 'models/451_r3-e5';
const sourceModelIndex = 11;

const _modelDirName = 'models/451_r4';
...
// const sourceModelDirName = `${_sourceModelDirName}-e${epochsValue}`;
...
const getNextDatasets = async ({ iterationIndex } = {}) => {
  ...
  if (typeof iterationIndex === 'undefined') {
  ...
  console.log('Using existing testData');

  const testDataFileName = path.resolve(sourceModelDirName, 'testDataUsed', `i${iterationIndex}.json`);
  const testData = JSON.parse(await fs.readFile(testDataFileName, 'utf8'));

  const testFens = testData.map(({ fen }) => fen);
  const trainData = records.filter(({ fen }) => !testFens.includes(fen));

  console.log({ tranLength: trainData.length, testLength: testData.length });
  return { trainData, testData, filesLoaded, remainingFiles: allDatasetFiles.length };
};
...
const run = async function () {
  ...
  const sourceTrainingMeta = JSON.parse(await fs.readFile(path.resolve(sourceModelDirName, 'trainingMeta.json')));
  const sourceModelFolder = sourceTrainingMeta.iterations[sourceModelIndex].folder;

  //   console.log({ sourceModelFolder });
  //   //   process.exit(0)
  //   return;

  const model = await loadModel({ folder: sourceModelFolder }); //buildModel();
  model.summary();

  let iterationIndex = 0;
  // eslint-disable-next-line no-constant-condition
  while (true) {
    let { finished } = await runIteration({ model, iterationIndex: iterationIndex++ });
    if (finished) break;
  }
