...
const initialSourceModelDirName = 'models/fromto_S_v1_0.00005/0.02597039-1677178848328';
// const initialSourceModelDirName = 'models/fromto_S_v1';
const targetModelName = 'models/fromto_S_v1';
...
const initialLearningRate = 0.00005; //0.0001; //0.001; //0.0005; //0.0005; //0.0005; //0.000125; //0.000015625; //0.001;
...
const maxIterationsWithoutImprovement = 40; //10;
...
const fileNamesToCopy = {
  'train.mjs': './trainFromto_v1.mjs',
  'loader.js': './dist/fromto_loader.js',
...
const transformRecord = (record) => {
  ...
  const [from, to] = oneHotMovesMap[Number(onehot_move)];
  ys[from] = 1;
  ys[to + 64] = 1;
...
const evaluateModel = async function ({ model, initial }) {
  ...
  result.isBetter = Number(result.binaryCrossentropy) <= Number(lossToBeat);

  oldLossToBeat = lossToBeat;

  if (initial) {
    lossToBeat = result.binaryCrossentropy;
  } else {
    await loadTestData();
    ({ binaryCrossentropy: lossToBeat } = await getLoss({ model }));
  }
...
const run = async function () {
  ...
  while (learningRate >= finalLearningRate) {
    ...
    do {
      ...
      const { binaryCrossentropy, isBetter } = await evaluateModel({ model });
      ...
      if (isBetter) {
        // if (/*typeof currentBest === 'undefined' ||*/ Number(binaryCrossentropy) <= Number(lossToBeat)) {
    ...
    await saveModel({
      model,
      binaryCrossentropy: 'lastForgotten',
...
const init = async ({ learningRate, modelDirName, sourceModelDirName }) => {
  try {
    ...
    if (!alreadyInited) {
      ...
      const { binaryCrossentropy } = await evaluateModel({ model, initial: true });
